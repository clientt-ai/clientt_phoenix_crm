defmodule ClienttCrmApp.Repo.Migrations.AddFormsAndAuthorizationDomains do
  ...
  final table naming convention (forms_*, forms_fields with tenant_id).
  """
  ...
  def up do
    ...
    # Create form_fields table with final name and tenant_id included
    ...
    # Create authz_tenant_settings table
    create table(:authz_tenant_settings, primary_key: false) do
      add :id, :uuid, null: false, default: fragment("gen_random_uuid()"), primary_key: true
      add :tenant_id, :uuid, null: false
    ...
    # Create authz_tenants table (shell first, then add columns)
    create table(:authz_tenants, primary_key: false) do
      add :id, :uuid, null: false, default: fragment("gen_random_uuid()"), primary_key: true
    end
    ...
    alter table(:forms_submissions) do
      modify :tenant_id,
             references(:authz_tenants,
               column: :id,
               name: "forms_submissions_tenant_id_fkey",
               type: :uuid,
               prefix: "public"
             )
    ...
    alter table(:forms_forms) do
      modify :tenant_id,
             references(:authz_tenants,
               column: :id,
               name: "forms_forms_tenant_id_fkey",
               type: :uuid,
               prefix: "public"
             )
    ...
    alter table(:forms_fields) do
      modify :tenant_id,
             references(:authz_tenants,
               column: :id,
               name: "forms_fields_tenant_id_fkey",
               type: :uuid,
               prefix: "public"
             )
    ...
    create unique_index(:forms_forms, [:tenant_id, :name],
             name: "forms_forms_unique_name_per_company_index"
           )

    create unique_index(:forms_forms, [:tenant_id, :slug],
    ...
    alter table(:authz_users) do
      modify :tenant_id,
             references(:authz_tenants,
               column: :id,
               name: "authz_users_tenant_id_fkey",
               type: :uuid,
               prefix: "public"
             )
    ...
    create unique_index(:authz_users, [:authn_user_id, :tenant_id],
    ...
    alter table(:authz_teams) do
      modify :tenant_id,
             references(:authz_tenants,
               column: :id,
               name: "authz_teams_tenant_id_fkey",
               type: :uuid,
               prefix: "public"
             )
    end

    create unique_index(:authz_teams, [:tenant_id, :name],
             name: "authz_teams_unique_name_per_company_index"
           )

    # Add authz_tenant_settings foreign keys
    alter table(:authz_tenant_settings) do
      modify :tenant_id,
             references(:authz_tenants,
               column: :id,
               name: "authz_tenant_settings_tenant_id_fkey",
               type: :uuid,
               prefix: "public"
             )
    end

    create unique_index(:authz_tenant_settings, [:tenant_id],
             name: "authz_tenant_settings_unique_company_index"
           )

    # Add authz_tenants columns
    alter table(:authz_tenants) do
      add :name, :text, null: false
      add :slug, :text, null: false
      add :status, :text, null: false, default: "active"
    ...
    create unique_index(:authz_tenants, [:slug], name: "authz_tenants_unique_slug_index")
  end

  def down do
    drop_if_exists unique_index(:authz_tenants, [:slug],
                     name: "authz_tenants_unique_slug_index"
                   )
    ...
    drop_if_exists unique_index(:authz_tenant_settings, [:tenant_id],
                     name: "authz_tenant_settings_unique_company_index"
                   )

    drop constraint(:authz_tenant_settings, "authz_tenant_settings_tenant_id_fkey")

    alter table(:authz_tenant_settings) do
      modify :tenant_id, :uuid
    end

    drop_if_exists unique_index(:authz_teams, [:tenant_id, :name],
                     name: "authz_teams_unique_name_per_company_index"
                   )

    drop constraint(:authz_teams, "authz_teams_tenant_id_fkey")

    alter table(:authz_teams) do
      modify :tenant_id, :uuid
    end

    drop_if_exists unique_index(:authz_users, [:authn_user_id, :tenant_id],
                     name: "authz_users_unique_user_company_index"
                   )

    drop constraint(:authz_users, "authz_users_tenant_id_fkey")
    ...
    drop_if_exists unique_index(:forms_forms, [:tenant_id, :slug],
                     name: "forms_forms_unique_slug_per_company_index"
                   )

    drop_if_exists unique_index(:forms_forms, [:tenant_id, :name],
                     name: "forms_forms_unique_name_per_company_index"
                   )

    drop constraint(:forms_fields, "forms_fields_tenant_id_fkey")
    ...
    drop constraint(:forms_forms, "forms_forms_tenant_id_fkey")
    ...
    drop constraint(:forms_submissions, "forms_submissions_tenant_id_fkey")
    ...
    drop table(:authz_tenants)

    drop table(:authz_tenant_settings)
