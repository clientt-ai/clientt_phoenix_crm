...
/**
 * Direct Links - Form Admin Role Screenshots
 *
 * This test captures screenshots of all pages when logged in as a form admin.
 * Tests both light and dark mode for each page.
 */

test.describe('Direct Links - Form Admin Role', () => {
  test.describe.configure({ mode: 'serial' });

  const screenshot = createScreenshotHelper(__dirname);
  const FORM_ADMIN_EMAIL = 'sample_form_admin@clientt.com';
  const FORM_ADMIN_PASSWORD = 'Hang123!';
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.fill('input[name="user[email]"]', FORM_ADMIN_EMAIL);
    await page.fill('input[name="user[password]"]', FORM_ADMIN_PASSWORD);
