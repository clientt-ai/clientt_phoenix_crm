...
/**
 * Direct Links - Admin Role Screenshots
 *
 * This test captures screenshots of all pages when logged in as an admin.
 * Tests both light and dark mode for each page.
 */

test.describe('Direct Links - Admin Role', () => {
  test.describe.configure({ mode: 'serial' });

  const screenshot = createScreenshotHelper(__dirname);
  const ADMIN_EMAIL = 'sample_admin@clientt.com';
  const ADMIN_PASSWORD = 'Hang123!';
  ...
  test.beforeAll(async ({ browser }) => {
    // Login once and save the auth state
