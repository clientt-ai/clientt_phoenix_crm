...
/**
 * Direct Links - Manager Role Screenshots
 *
 * This test captures screenshots of all pages when logged in as a manager.
 * Tests both light and dark mode for each page.
 */

test.describe('Direct Links - Manager Role', () => {
  test.describe.configure({ mode: 'serial' });

  const screenshot = createScreenshotHelper(__dirname);
  const MANAGER_EMAIL = 'sample_manager@clientt.com';
  const MANAGER_PASSWORD = 'Hang123!';
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.fill('input[name="user[email]"]', MANAGER_EMAIL);
    await page.fill('input[name="user[password]"]', MANAGER_PASSWORD);
